...
3. Open **Preview & Settings** only as an option. Confirm its usage instructions and one-complete-chapter limit are clear, the full Lockman notice is visible, and closing the pane does not stop an already active runtime or later ribbon Fill.
...
5. Confirm `Psalm 119:1-176` is accepted and `Psalm 119:1-177` is refused locally without a document change. Repeat with duplicate references, clear the local Scripture cache, and confirm a later activation refetches rather than changing prose early.
...
- One reference may contain any valid range within one chapter, including Psalm 119:1–176.
