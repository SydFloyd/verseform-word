...
WordApi 1.6 provides paragraph-change events and stable-per-session paragraph IDs. WordApi 1.7 provides temporary critique annotations plus hover, click, insertion, and removal events. VFW-010 first proved an accessible task-pane preview and Insert action. VFW-040's `D-019` high-throughput refinement maps hover to preview-only behavior and maps click or `Alt+Down` to the existing guarded insertion command, retaining the task-pane control as an alternative. A ready or in-flight preview for that exact annotation and translation is reused so an immediate click does not duplicate the DBS request. WordApi 1.8 popup options are not used as arbitrary command buttons: their suggestions are Word-owned document replacements and would bypass Verseform's exact final range command. The Word on the web walk proved click and `Alt+Down`; trusted pointer movement did not produce a hover event there, so the hover handler remains best effort under `D-014`. Event handlers exist only while the add-in runtime is alive and must be registered again after reopening.
...
The final VFW-010 host pass proved unsupported-host messaging, cross-document operation, pane close/reopen recovery, and a post-close Undo that returned to a current safe preview. The prior opaque-ID refusal remains deterministic proof for the alternate resurrection branch Word did not surface. `D-014` removes hover delivery from the required interaction because the real web host did not deliver it under trusted pointer movement; `D-019` makes click and `Alt+Down` the explicit insertion paths while the pane remains an alternative. Screen-reader and forced-color operation are release-host validation. The non-atomic check-then-replace coauthor timing limitation is accepted and disclosed by `D-013`; it is not an atomicity claim.

Verseform maps each candidate by inserting one critique at a time and recording its returned annotation ID, rather than assuming a result-array ordering for duplicates. Hover selects that exact ID and previews without mutation when the host delivers it. Click or Word's `Alt+Down` command selects the exact ID, awaits its existing preview request or starts one, and then enters the same insertion command exposed by the task pane. During replacement, the Word adapter reloads the paragraph and the annotation's own range, source text, critique offset, critique length, and owning paragraph before queuing one `Range.insertText(..., "Replace")` mutation. It refuses unknown, missing, stale, or ambiguous mappings.
...
Detection never calls this port. Hover or explicit insertion may start a cancellable request; a click reuses an in-flight hover request for the same annotation and translation rather than issuing another. Translation changes, paragraph changes, cancellation, runtime stop, and a different activation invalidate the request stamp, and a result is accepted only when the annotation, paragraph revision/source, translation, and request generation still match. Click-to-insert proceeds only after that preview is ready and then performs the independent final Word reread and exact range check. No WEB or other fallback Bible exists.
