...
A catalog request may load the translations authorized by Digital Bible Society. An uncached optional preview or insertion may request only the selected translation identifier, canonical book identifier, and chapter number from the DBS ARC API. Hover while the optional pane is hidden makes no provider request. Provider responses are treated as untrusted data and never executed as HTML.
...
No Bible is bundled. Without DBS connectivity, reference detection may continue but passage preview and insertion are unavailable. Inserted quotations retain the translation abbreviation in their citation; the optional pane displays the selected translation's complete provider copyright notice.
